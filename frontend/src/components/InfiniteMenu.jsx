...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div className="infinite-menu-wrap" style={{ position: 'relative', width: '100%', height: '100%' }} data-testid="infinite-menu">
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${isMoving ? 'inactive' : 'active'}`} data-testid="infinite-menu-title">{activeItem.title}</h2>
          <p className={`face-description ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</p>
          <button type="button" onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'}`} aria-label="Open profile">
